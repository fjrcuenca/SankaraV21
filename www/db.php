<?php
	...
	function add_student() {
		...
		$qry = 'INSERT INTO students (firstname, lastname, screening_site, gender, birth_date,
			...
			fundux_l, iop_l, duct_l, amblyopia_l, notes, camp, anganwadi) values ("' . $firstname . '","' . $lastname . '","' . $screening_site . '"
...
/** NEED TO FIX **/	
	...
	function delete_student() {
		...
		$school_name = $_GET['school_name'];
		if($site == 'school') {
			$del = mysql_query("DELETE FROM students WHERE school = '$site_name' AND firstname = '$firstname' AND lastname = '$lastname' AND parent_name = '$parent_name'");
		} else if ($site == 'camp') {
			$del = mysql_query("DELETE FROM students WHERE camp = '$site_name' AND firstname = '$firstname' AND lastname = '$lastname' AND parent_name = '$parent_name'");
		} else {
			$del = mysql_query("DELETE FROM students WHERE anganwadi = '$site_name' AND firstname = '$firstname' AND lastname = '$lastname' AND parent_name = '$parent_name'");
		}
		if($del)
			return true;
		return false;     
